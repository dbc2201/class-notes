...
describe('Input Component', () => {
	...
	describe('Interactions', () => {
		...
		it('handles typing in email input', () => {
			const onChange = vi.fn();
			const props = makeProps({ type: 'email', onChange, label: 'Email' });
			render(<Input {...props} />);
			...
			expect(onChange).toHaveBeenCalledTimes(1);
			expect((onChange.mock.calls[0][0].target as HTMLInputElement).value).toBe(
				'test@example.com'
			);
		});
		...
		it('handles typing in password input', () => {
			const onChange = vi.fn();
			const props = makeProps({ type: 'password', onChange, label: 'Password' });
			render(<Input {...props} />);
		...
		it('handles typing in number input', () => {
			const onChange = vi.fn();
			const props = makeProps({ type: 'number', onChange, label: 'Age' });
			render(<Input {...props} />);
